...
class EmoProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = line[0]
            label = list(line[1].split(","))[0] # only consider one label for now
            # if i % 5000 == 0:
            #     logger.info(line)
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, type="train", num_show=3,
    ...
    for (_, example) in enumerate(examples):
        ...
        if example.rand_index_a != None:
            # print("rand_index_a: {}".format(example.rand_index_a))
        ...
        input_mask += padding # attention mask
        segment_ids += padding
        ...
        if ex_index < num_show:
            logger.info("*** {} Example ***".format(type))
...
def aug_func(ori_examples, label_list, tokenizer, cnt, num_no_aug, args, seed, max_seq_length, output_mode, num_show,
    ...
    for (guid, example) in enumerate(ori_examples):
        tokens_a = tokenizer.tokenize(example.text_a)
        if not tokens_a:
            logger.info("guid: {} {}".format(guid, example.guid))
            logger.info("tokens: {}".format(
                [str(x) for x in tokens]))
            logger.info("text: {}".format(example.text_a))
            logger.info("sequence_label: {} (id = {})".format(example.label, label_id))
        ...
        for i in range(cnt):
            ...
            if i >= num_no_aug:  # first several time is original
                ...
                # print("text_a: {}, tokens_a: {}".format(example.text_a, tokens_a))
...
def Aug_each_ckpt(ori_examples, label_list, model, tokenizer, args=None, num_show=5, output_mode="classification",
    ...
    while len(total_auged_ex) < len_aug:
        ...
        if output_mode == "classification":
            ...
            # print("[Aug_each_ckpt]all_label_ids: {}".format(all_label_ids))
        elif output_mode == "regression":
