...
class BertSeqAttention(BertPreTrainedModel):
    ...
    def forward(self, sequence_output, attention_mask, token_real_label, aug_logits, pooled_output):
        ...
        # aug_logits: torch.Size([32, 128, 2])
        ...
        # attention_probs torch.Size([32, 128, 2])
        attention_probs = attention_probs[:, :, 1]
        # attention_probs torch.Size([32, 128])
        # print("attention_probs: {}".format(attention_probs))
...
class BertTokenAug(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        # print("[BertTokenAug]bert_model_embedding_weights: {}".format(bert_model_embedding_weights.size()))
    ...
    def forward(self, sequence_output):
        ...
        # sequence output: (batch_size, max_seq_length, hidden_size) == hidden_state
        # scores: 32, 128, 2
        # print("[BertTokenAug] sequence_output: {}, hidden_states: {}, scores: {}".format(
...
class BertForNSPAug(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, token_real_label=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                   output_all_encoded_layers=False,
                                                   token_real_label=token_real_label)
        # pooled_output: CLS->(batch_size, hidden_size)
        ...
        # print("[BertForNSPAug]aug_logits: {}, seq_logits: {}, token_real_label: {}".format(
        #     aug_logits.size(),
        #     seq_logits.size(),
        #     token_real_label.view(-1).size()))
        # aug_logits: torch.Size([32, 128, 2]), 
        # seq_logits: torch.Size([32, 28]), 
        # token_real_label: torch.Size([4096]) -> 32 * 128
