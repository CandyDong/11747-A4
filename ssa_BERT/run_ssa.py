...
def acc_and_f1(preds, labels, average_type="binary"):
	acc = simple_accuracy(preds, labels)
	# f1 = f1_score(y_true=labels, y_pred=preds)
	f1 = f1_score(y_true=labels, y_pred=preds, average = average_type)
	return {
		"acc": acc,
		"f1": f1,
		"labels": np.unique(labels),
		"all_f1": f1_score(y_true=labels, y_pred=preds, average = None),
		"acc_and_f1": (acc + f1) / 2,
	}
...
def compute_metrics(task_name, preds, labels):
	...
	if task_name == "cola":
		...
		return acc_and_f1(preds, labels, average_type="macro")
	else:
...
def do_evaluate(args, processor, label_list, tokenizer, model, epoch, output_mode, num_labels, task_name, eval_examples, type="dev"):
	...
	logger.info("  Num {} examples = {}".format(type, len(eval_examples)))
	...
	for kk in range(max_n):
		...
		logger.info("************************************ [%d] Running %s ***********************************", kk, type)
		logger.info("  Num {} examples = %d".format(type, len(eval_features)))
		logger.info("  Batch size = %d", args.eval_batch_size)
		...
		if type == "dev":
			...
			if kk == 0:
				...
				for key in res:
					if key == "all_f1" or key == "labels":
						continue
					res_all[key]+=res[key]

		logger.info("************************************ [%d] %s done ***********************************", kk, type)
